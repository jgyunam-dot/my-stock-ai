...
다음 조건을 모두 충족하는 종목만 추천하세요:
- 보유 종목과 완전히 다른 종목
- 현재 시장 흐름(지수, 환율, 나스닥)과 연동성이 높은 종목
- 단기 모멘텀이 살아있는 종목 (거래량 증가, 섹터 테마 부합)
- 리스크 대비 수익률이 검증된 대형주 또는 성장 중소형주
- {target_period} 시장 흐름에 맞는 종목

아래 JSON 형식으로만 출력하세요. 다른 말은 절대 하지 마세요:
[
  {{"name": "종목명", "code": "종목코드.KS또는.KQ", "reason": "추천이유 (섹터, 모멘텀, 시장흐름 연관성 포함)"}},
  {{"name": "종목명", "code": "종목코드.KS또는.KQ", "reason": "추천이유 (섹터, 모멘텀, 시장흐름 연관성 포함)"}},
  {{"name": "종목명", "code": "종목코드.KS또는.KQ", "reason": "추천이유 (섹터, 모멘텀, 시장흐름 연관성 포함)"}}
]
"""
                ...
                with st.spinner("🤖 AI 최종 분석 중..."):
                    prompt = f"""
당신은 대한민국 주식시장에서 30년 이상 활동한 최고 수익률의 전문 트레이더입니다.
코스피/코스닥 상장 이후 수많은 사이클을 직접 경험했으며,
...
- 근거: 시장 흐름, 섹터 상황, 수급 관점에서 2~3줄
- 매수 시점: 구체적인 가격 또는 조건 (현재가 기준)
- 매도 시점: 목표가 또는 손절가 (현재가 기준 %)
...
- 추천 근거: 섹터 테마, 모멘텀, 시장 연동성 포함 3~4줄
- 매수 시점: 현재가 기준 구체적 가격
- 1차 목표가: 현재가 기준 %
- 2차 목표가: 현재가 기준 %
- 손절가: 현재가 기준 %
- 투자 기간: 단기(1주일 이내) / 중기(1개월) / 장기(3개월 이상)
